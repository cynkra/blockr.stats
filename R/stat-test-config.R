...
test_categories <- list(
  ...
  categorical = list(label = "Categorical", icon = "grid-3x3")
)
...
test_config <- list(
  ...
  t_test_paired = list(
    ...
    params = list(
      alternative = list(
        type = "select", label = "Alternative",
        choices = c("Two sided" = "two.sided", "Greater" = "greater",
                    "Less" = "less"),
        default = "two.sided", advanced = TRUE
      ),
      conf_level = list(
        type = "numeric", label = "Confidence level",
        default = 0.95, min = 0, max = 1, step = 0.01, advanced = TRUE
      ),
      null = list(
        type = "numeric", label = "Null difference",
        default = 0, step = 0.1, advanced = TRUE
      )
    ),
    test_fn = "test_t_paired"
  ),
  wilcoxon_paired = list(
    ...
    params = list(
      ...
      null = list(
        type = "numeric", label = "Null location shift",
        default = 0, step = 0.1, advanced = TRUE
      )
    ),
    test_fn = "test_wilcoxon_paired"
  ),
  chisq_independence = list(
    category = "categorical",
    label = "Chi-square test of independence",
    icon = "grid-3x3",
    inputs = list(
      values = list(role = "required", type = "factor", multiple = TRUE),
      groups = list(role = "required", type = "factor")
    ),
    params = list(),
    test_fn = "test_chisq_independence"
  )
)
