...
#' Paired t-test (first two value columns are the paired measurements)
#' @noRd
test_t_paired <- function(data, values, groups, params) {
  alternative <- params$alternative %||% "two.sided"
  conf_level  <- params$conf_level %||% 0.95
  mu          <- params$null %||% 0
  if (length(values) < 2) {
    warning("Paired t-test needs two value columns.")
    return(dplyr::bind_rows(list(tibble::tibble_row(
  ...
  dplyr::bind_rows(list(tibble::tibble_row(
    method    = trimws(res$method),
    values    = paste(values[1], "-", values[2]),
    n         = length(x),
...
#' @noRd
test_wilcoxon_paired <- function(data, values, groups, params) {
  alternative <- params$alternative %||% "two.sided"
  mu          <- params$null %||% 0
  if (length(values) < 2) {
    warning("Signed-rank test needs two value columns.")
    return(dplyr::bind_rows(list(tibble::tibble_row(
      method = NA_character_, values = NA_character_, n = 0L,
      statistic = NA_real_, p.value = NA_real_))))
  }
...
#' @noRd
test_chisq_independence <- function(data, values, groups, params) {
  results <- list()
  for (vcol in values) {
    for (gcol in groups) {
      keep <- !is.na(data[[vcol]]) & !is.na(data[[gcol]])
      tab <- table(data[[vcol]][keep], data[[gcol]][keep])
      if (nrow(tab) < 2 || ncol(tab) < 2) {
        warning("Chi-square skipped for '", vcol, "' x '", gcol,
                "': need >= 2 levels each.")
        results <- c(results, list(tibble::tibble_row(
          method = NA_character_, values = vcol, groups = gcol,
          n = sum(tab), statistic = NA_real_, df = NA_real_,
          p.value = NA_real_)))
        next
      }
